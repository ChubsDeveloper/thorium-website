 ...
 * Handles both site and auth database login with CSRF protection
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if (!function_exists('csrf_check') || !csrf_check($_POST['csrf'] ?? '')) {
    ...
    if (!$pdo || !$authPdo) {
      ...
      if (function_exists('login_any') && ($user = login_any($pdo, $authPdo, $config, $u, $p, $adminMin))) {
        $dest = $next ?: (function_exists('base_url') ? base_url('') : '/');
  ...
  <div class="mx-auto max-w-md">
    <div class="rough-card p-6 md:p-8">
      ...
      <div class="mt-4 text-sm flex flex-wrap items-center justify-between gap-2 text-neutral-300">
        <div class="space-x-3">
          <a class="text-amber-300 hover:text-ember-500" href="<?= e(function_exists('base_url') ? base_url('forgot') : '/forgot') ?>">Forgot password?</a>
          <a class="text-amber-300 hover:text-ember-500" href="<?= e(function_exists('base_url') ? base_url('forgot-username') : '/forgot-username') ?>">Forgot username?</a>
        </div>
        <a class="btn-ghost btn-sm" href="<?= e(function_exists('base_url') ? base_url('register') : '/register') ?>">Create account</a>
      </div>
    </div>
  </div>
